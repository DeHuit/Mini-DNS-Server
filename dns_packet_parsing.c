 ...
 */
void encode_domain_name_string(char * domain_name, unsigned char * encoded_name, int * size) {
...
int build_dns_request(char * message, char * name) {
  ...
  unsigned char * encoded_name = malloc(strlen(name) + 5);
  int size;
  encode_domain_name_string(name, encoded_name, &size);
  /* ID */
  message[0] = (rand() % 128);
  message[1] = (rand() % 128);
...
int main(int argc, char const *argv[]) {
  ...
  int a = build_dns_request(msg, "univ-tlse3.fr");
  print_dns_packet(msg, a);
  return 0;
}*/
